/* CM Banquetería · experiencia administrativa diaria, tablas desplegables y contraste v48 */
...
operations=async function(){
  ...
  content.innerHTML=`<section class="cm-v47-page-intro"><div><div class="kicker">RESTAURANT</div><h3>Reservas, retiros y comunicación con cocina</h3><p>Ordena la información de la jornada y compártela con el equipo mediante la pantalla interna. Cada acción tiene un lugar claro y un estado visible.</p></div><a class="cm-v47-screen-link" href="/pantalla.html" target="_blank" rel="noopener"><span class="material-symbols-rounded">desktop_windows</span><div><strong>Abrir pantalla de cocina</strong><small>Vista de alto contraste para tablet o TV</small></div></a></section>
...
suppliers=async function(){
  ...
  const rows=data.items.map(s=>`<tr data-cm-supplier-row="${safe([s.name,s.rut,s.business_type,s.usual_products,s.contact_name].filter(Boolean).join(' ').toLowerCase())}"><td><strong>${safe(s.name)}</strong></td><td>${safe(s.rut||'')}</td><td>${safe(s.business_type||'')}</td><td>${safe(s.usual_products||'')}</td><td>${supplierContactSummary(s)}</td><td>${fmtDate(s.last_purchase_date)}</td><td class="row-actions"><button class="btn btn-secondary btn-small" type="button" onclick="openSupplierDetail(${s.id})">Ficha</button><button class="btn btn-secondary btn-small" type="button" onclick="editSupplier(${s.id})">Editar</button></td></tr>`);
  content.innerHTML=`<section class="cm-v47-page-intro"><div><div class="kicker">GESTIÓN INTERNA</div><h3>Proveedores habituales</h3><p>Mantén contacto, rubro, productos frecuentes e historial de cada proveedor en una ficha fácil de revisar y descargar.</p></div><a class="cm-v47-excel-link" href="/api/admin/suppliers.xlsx"><span class="material-symbols-rounded">table_view</span><div><strong>Descargar tabla Excel</strong><small>Proveedores y antecedentes</small></div></a></section><section class="cm-v47-supplier-tools"><label><span class="material-symbols-rounded">search</span><input id="cmSupplierSearch" type="search" placeholder="Buscar por proveedor, RUT, rubro o producto" oninput="filterCmV47Suppliers(this.value)"></label><button class="btn btn-primary" type="button" onclick="openSupplierForm()">Agregar proveedor</button></section><section id="cmV47SupplierGrid" class="cm-v47-supplier-grid">${cards}</section><section class="cm-v48-table-reveal"><div><span>VISTA COMPLETA</span><strong>Tabla de proveedores</strong><small>Ábrela para comparar contactos, productos y antecedentes en una sola vista.</small></div><button class="btn btn-secondary cm-v48-table-toggle" type="button" aria-expanded="false" aria-controls="cmSupplierTable" onclick="toggleCmV48Table('cmSupplierTable',this)"><span class="material-symbols-rounded">table_rows</span><span>Ver en modo tabla</span></button></section><section id="cmSupplierTable" class="cm-v47-content-panel cm-v48-collapsible-table hidden"><header><div><span>VISTA TABULAR</span><h3>Control completo de proveedores</h3></div></header>${table(['Proveedor','RUT','Rubro','Productos habituales','Contacto','Última compra','Acciones'],rows)}</section>`;
};
window.toggleCmV48Table=(id,button)=>{const panel=document.getElementById(id);if(!panel)return;const willOpen=panel.classList.contains('hidden');panel.classList.toggle('hidden',!willOpen);button?.setAttribute('aria-expanded',willOpen?'true':'false');const label=button?.querySelector('span:last-child');if(label)label.textContent=willOpen?'Ocultar tabla':'Ver en modo tabla';if(willOpen)panel.scrollIntoView({behavior:'smooth',block:'start'});};
