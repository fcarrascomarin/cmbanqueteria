...
-- Funciones iniciales del equipo CM según registros operativos de limpieza y sanitización.
UPDATE staff SET role=COALESCE(NULLIF(role,''),'Administradora'), assigned_tasks=CASE WHEN assigned_tasks IS NULL OR BTRIM(assigned_tasks)='' THEN 'Administración general; coordinación del equipo; caja; seguimiento de costos y cotizaciones; organización de la carpeta sanitaria y documental; supervisión del cumplimiento de registros operativos.' ELSE assigned_tasks END, notes=CASE WHEN notes IS NULL OR BTRIM(notes)='' OR notes LIKE 'Registro inicial de equipo CM%' THEN 'Responsable administrativa de CM. Completar RUT, teléfono, jornada, contrato y documentación asociada.' ELSE notes END WHERE LOWER(full_name)=LOWER('Claudia Mendez');
UPDATE staff SET role=COALESCE(NULLIF(role,''),'Maestra de cocina'), assigned_tasks=CASE WHEN assigned_tasks IS NULL OR BTRIM(assigned_tasks)='' THEN 'Producción y supervisión de cocina; limpieza y sanitización del horno cada vez que se utilice; lavado de utensilios, ollas y herramientas de cocina junto al equipo; control de la preparación segura del servicio.' ELSE assigned_tasks END, notes=CASE WHEN notes IS NULL OR BTRIM(notes)='' OR notes LIKE 'Registro inicial de equipo CM%' THEN 'Tareas complementadas desde el Programa de limpieza y sanitización de cocina y bodega.' ELSE notes END WHERE LOWER(full_name)=LOWER('Silvia');
UPDATE staff SET role=COALESCE(NULLIF(role,''),'Ayudante de cocina'), assigned_tasks=CASE WHEN assigned_tasks IS NULL OR BTRIM(assigned_tasks)='' THEN 'Apoyo en preparación; limpieza diaria de la cocina de dos platos; limpieza de la campana los lunes y jueves; lavado de utensilios, ollas y herramientas de cocina junto al equipo.' ELSE assigned_tasks END, notes=CASE WHEN notes IS NULL OR BTRIM(notes)='' OR notes LIKE 'Registro inicial de equipo CM%' THEN 'En el programa de limpieza la responsable aparece escrita como “Yeny”; confirmar que corresponde a Jenny al completar la ficha laboral.' ELSE notes END WHERE LOWER(full_name)=LOWER('Jenny');
UPDATE staff SET role=COALESCE(NULLIF(role,''),'Aseo comedor y apoyo en cocina'), assigned_tasks=CASE WHEN assigned_tasks IS NULL OR BTRIM(assigned_tasks)='' THEN 'Aseo y apoyo operacional; limpieza de mesas, individuales, alcuzas y saleros cuando sea necesario; apoyo en lavado de utensilios, ollas y herramientas de cocina; apoyo en orden y sanitización de áreas de servicio.' ELSE assigned_tasks END, notes=CASE WHEN notes IS NULL OR BTRIM(notes)='' OR notes LIKE 'Registro inicial de equipo CM%' THEN 'Tareas complementadas desde los programas de limpieza de comedores y cocina/bodega.' ELSE notes END WHERE LOWER(full_name)=LOWER('Marlen');
UPDATE staff SET role=COALESCE(NULLIF(role,''),'Mesera part-time'), assigned_tasks=CASE WHEN assigned_tasks IS NULL OR BTRIM(assigned_tasks)='' THEN 'Atención de mesas; limpieza de mesas, individuales, alcuzas y saleros; lavado profundo de alcuzas y saleros los viernes y limpieza exterior diaria; limpieza diaria de pisos de comedor, terraza y sector tinaja; limpieza diaria de refrigeradores del comedor; apoyo en lavado de utensilios.' ELSE assigned_tasks END, notes=CASE WHEN notes IS NULL OR BTRIM(notes)='' OR notes LIKE 'Registro inicial de equipo CM%' THEN 'Tareas complementadas desde el Programa de limpieza y sanitización de comedores.' ELSE notes END WHERE LOWER(full_name)=LOWER('Sofia');
