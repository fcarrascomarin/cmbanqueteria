...
ALTER TABLE staff ADD COLUMN IF NOT EXISTS assigned_tasks TEXT;
...
// Generador XLSX liviano sin dependencias externas. Produce una hoja con encabezado,
// bordes, filtros y anchos de columna adecuados para uso administrativo.
const CM_XLSX_CRC_TABLE=(()=>{const table=new Uint32Array(256);for(let n=0;n<256;n++){let c=n;for(let k=0;k<8;k++)c=(c&1)?0xEDB88320^(c>>>1):c>>>1;table[n]=c>>>0}return table})();
function cmXlsxCrc32(buffer){let c=0xFFFFFFFF;for(const byte of buffer)c=CM_XLSX_CRC_TABLE[(c^byte)&0xFF]^(c>>>8);return (c^0xFFFFFFFF)>>>0}
function cmXlsxDosTime(date=new Date()){const year=Math.max(1980,date.getFullYear()),dosDate=((year-1980)<<9)|((date.getMonth()+1)<<5)|date.getDate(),dosTime=(date.getHours()<<11)|(date.getMinutes()<<5)|(date.getSeconds()>>1);return {dosDate,dosTime}}
function cmXlsxZip(files){const local=[],central=[];let offset=0;const now=cmXlsxDosTime();for(const file of files){const name=Buffer.from(file.name,'utf8'),data=Buffer.isBuffer(file.data)?file.data:Buffer.from(file.data,'utf8'),crc=cmXlsxCrc32(data);const lh=Buffer.alloc(30);lh.writeUInt32LE(0x04034b50,0);lh.writeUInt16LE(20,4);lh.writeUInt16LE(0x0800,6);lh.writeUInt16LE(0,8);lh.writeUInt16LE(now.dosTime,10);lh.writeUInt16LE(now.dosDate,12);lh.writeUInt32LE(crc,14);lh.writeUInt32LE(data.length,18);lh.writeUInt32LE(data.length,22);lh.writeUInt16LE(name.length,26);lh.writeUInt16LE(0,28);local.push(lh,name,data);const ch=Buffer.alloc(46);ch.writeUInt32LE(0x02014b50,0);ch.writeUInt16LE(20,4);ch.writeUInt16LE(20,6);ch.writeUInt16LE(0x0800,8);ch.writeUInt16LE(0,10);ch.writeUInt16LE(now.dosTime,12);ch.writeUInt16LE(now.dosDate,14);ch.writeUInt32LE(crc,16);ch.writeUInt32LE(data.length,20);ch.writeUInt32LE(data.length,24);ch.writeUInt16LE(name.length,28);ch.writeUInt16LE(0,30);ch.writeUInt16LE(0,32);ch.writeUInt16LE(0,34);ch.writeUInt16LE(0,36);ch.writeUInt32LE(0,38);ch.writeUInt32LE(offset,42);central.push(ch,name);offset+=lh.length+name.length+data.length}const centralData=Buffer.concat(central),end=Buffer.alloc(22);end.writeUInt32LE(0x06054b50,0);end.writeUInt16LE(0,4);end.writeUInt16LE(0,6);end.writeUInt16LE(files.length,8);end.writeUInt16LE(files.length,10);end.writeUInt32LE(centralData.length,12);end.writeUInt32LE(offset,16);end.writeUInt16LE(0,20);return Buffer.concat([...local,centralData,end])}
function cmXlsxXml(value){return String(value??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&apos;'}[ch]))}
function cmXlsxCol(index){let n=index+1,out='';while(n){n--;out=String.fromCharCode(65+n%26)+out;n=Math.floor(n/26)}return out}
function cmXlsxWorkbook(sheetName,rows){const safeName=String(sheetName||'Datos').slice(0,31).replace(/[\\/*?:\[\]]/g,' ')||'Datos';const widths=(rows[0]||[]).map((_,c)=>Math.min(48,Math.max(11,...rows.map(r=>String(r[c]??'').length+2))));const sheetRows=rows.map((row,r)=>`<row r="${r+1}">${row.map((value,c)=>{const ref=`${cmXlsxCol(c)}${r+1}`,style=r===0?1:2;if(typeof value==='number'&&Number.isFinite(value))return `<c r="${ref}" s="${style}"><v>${value}</v></c>`;return `<c r="${ref}" s="${style}" t="inlineStr"><is><t xml:space="preserve">${cmXlsxXml(value)}</t></is></c>`}).join('')}</row>`).join('');const endRef=`${cmXlsxCol(Math.max(0,(rows[0]||[]).length-1))}${Math.max(1,rows.length)}`;const cols=widths.map((w,i)=>`<col min="${i+1}" max="${i+1}" width="${w}" customWidth="1"/>`).join('');const files=[
...
{name:'xl/workbook.xml',data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="${cmXlsxXml(safeName)}" sheetId="1" r:id="rId1"/></sheets></workbook>`},
{name:'xl/_rels/workbook.xml.rels',data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>`},
{name:'xl/styles.xml',data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><name val="Aptos"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Aptos"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF6B0B1B"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border><left/><right/><top/><bottom/><diagonal/></border><border><left style="thin"><color rgb="FFD9CEC5"/></left><right style="thin"><color rgb="FFD9CEC5"/></right><top style="thin"><color rgb="FFD9CEC5"/></top><bottom style="thin"><color rgb="FFD9CEC5"/></bottom><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="3"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" xfId="0" applyBorder="1" applyAlignment="1"><alignment vertical="top" wrapText="1"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`},
{name:'xl/worksheets/sheet1.xml',data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>${cols}</cols><sheetData>${sheetRows}</sheetData><autoFilter ref="A1:${endRef}"/><pageMargins left="0.3" right="0.3" top="0.5" bottom="0.5" header="0.2" footer="0.2"/></worksheet>`}
];return cmXlsxZip(files)}
function cmSendXlsx(res,fileName,sheetName,rows){const buffer=cmXlsxWorkbook(sheetName,rows);res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');res.setHeader('Content-Disposition',`attachment; filename="${fileName.replace(/[^A-Za-z0-9._-]/g,'_')}"`);res.setHeader('Content-Length',buffer.length);res.send(buffer)}
...
async function seedInitialStaff(db=pool){
  const staffBase=[
    {
      fullName:'Claudia Mendez',
      role:'Administradora',
      tasks:'Administración general; coordinación del equipo; caja; seguimiento de costos y cotizaciones; organización de la carpeta sanitaria y documental; supervisión del cumplimiento de registros operativos.',
      notes:'Responsable administrativa de CM. Completar RUT, teléfono, jornada, contrato y documentación asociada.'
    },
    {
      fullName:'Silvia',
      role:'Maestra de cocina',
      tasks:'Producción y supervisión de cocina; limpieza y sanitización del horno cada vez que se utilice; lavado de utensilios, ollas y herramientas de cocina junto al equipo; control de la preparación segura del servicio.',
      notes:'Tareas complementadas desde el Programa de limpieza y sanitización de cocina y bodega.'
    },
    {
      fullName:'Jenny',
      role:'Ayudante de cocina',
      tasks:'Apoyo en preparación; limpieza diaria de la cocina de dos platos; limpieza de la campana los lunes y jueves; lavado de utensilios, ollas y herramientas de cocina junto al equipo.',
      notes:'En el programa de limpieza la responsable aparece escrita como “Yeny”; confirmar que corresponde a Jenny al completar la ficha laboral.'
    },
    {
      fullName:'Marlen',
      role:'Aseo comedor y apoyo en cocina',
      tasks:'Aseo y apoyo operacional; limpieza de mesas, individuales, alcuzas y saleros cuando sea necesario; apoyo en lavado de utensilios, ollas y herramientas de cocina; apoyo en orden y sanitización de áreas de servicio.',
      notes:'Tareas complementadas desde los programas de limpieza de comedores y cocina/bodega.'
    },
    {
      fullName:'Sofia',
      role:'Mesera part-time',
      tasks:'Atención de mesas; limpieza de mesas, individuales, alcuzas y saleros; lavado profundo de alcuzas y saleros los viernes y limpieza exterior diaria; limpieza diaria de pisos de comedor, terraza y sector tinaja; limpieza diaria de refrigeradores del comedor; apoyo en lavado de utensilios.',
      notes:'Tareas complementadas desde el Programa de limpieza y sanitización de comedores.'
    }
  ];
  for(const person of staffBase){
    const inserted=await db.query(`INSERT INTO staff(full_name,role,status,assigned_tasks,notes)
      SELECT $1,$2,'activo',$3,$4
      WHERE NOT EXISTS (SELECT 1 FROM staff WHERE LOWER(full_name)=LOWER($1) AND LOWER(COALESCE(role,''))=LOWER($2))
      RETURNING id`,[person.fullName,person.role,person.tasks,person.notes]);
    if(!inserted.rowCount){
      await db.query(`UPDATE staff SET
        role=COALESCE(NULLIF(role,''),$2),
        assigned_tasks=CASE WHEN assigned_tasks IS NULL OR BTRIM(assigned_tasks)='' THEN $3 ELSE assigned_tasks END,
        notes=CASE WHEN notes IS NULL OR BTRIM(notes)='' OR notes LIKE 'Registro inicial de equipo CM%' THEN $4 ELSE notes END
        WHERE LOWER(full_name)=LOWER($1)`,[person.fullName,person.role,person.tasks,person.notes]);
    }
  }
}
...
app.get('/api/admin/staff.xlsx',auth,async(req,res)=>{
  try{
    const r=await pool.query(`SELECT full_name,role,rut,phone,start_date,contract_type,schedule,status,assigned_tasks,notes FROM staff ORDER BY full_name`);
    const rows=[['Nombre','Cargo','RUT','Teléfono','Fecha de ingreso','Tipo de contrato','Jornada / horario','Estado','Funciones y tareas asignadas','Observaciones'],...r.rows.map(x=>[x.full_name||'',x.role||'',x.rut||'',x.phone||'',x.start_date?String(x.start_date).slice(0,10):'',x.contract_type||'',x.schedule||'',x.status||'',x.assigned_tasks||'',x.notes||''])];
    cmSendXlsx(res,'CM_personal.xlsx','Personal CM',rows);
  }catch(e){console.error(e);res.status(500).json({error:'No se pudo generar el Excel de personal.'})}
});
app.get('/api/admin/suppliers.xlsx',auth,async(req,res)=>{
  try{
    const r=await pool.query(`SELECT s.*,COUNT(DISTINCT p.id)::int purchase_count,COALESCE(SUM(p.total) FILTER(WHERE p.status='confirmado' AND p.document_type<>'comprobante_pago'),0)::int purchase_total,MAX(p.purchase_date) last_purchase_date FROM suppliers s LEFT JOIN purchase_documents p ON p.supplier_id=s.id GROUP BY s.id ORDER BY s.name`);
    const rows=[['Proveedor','RUT','Rubro / tipo de venta','Productos habituales','Persona de contacto','Teléfono','Correo','Compras confirmadas','Total histórico','Última compra','Observaciones'],...r.rows.map(x=>[x.name||'',x.rut||'',x.business_type||'',x.usual_products||'',x.contact_name||'',x.phone||'',x.email||'',Number(x.purchase_count||0),Number(x.purchase_total||0),x.last_purchase_date?String(x.last_purchase_date).slice(0,10):'',x.notes||''])];
    cmSendXlsx(res,'CM_proveedores.xlsx','Proveedores CM',rows);
  }catch(e){console.error(e);res.status(500).json({error:'No se pudo generar el Excel de proveedores.'})}
});
app.get('/api/admin/daily-financials.xlsx',auth,async(req,res)=>{
  try{
    const month=monthParam(req.query.month),items=await dailyFinancialRows(month);
    const rows=[['Fecha','Clientes aproximados','Costo de alimentos','Ingreso','Costo %','Personal','Gastos básicos','Neto','Observaciones','Detalle de insumos'],...items.map(x=>[String(x.financial_date).slice(0,10),Number(x.customers_count||0),Number(x.food_cost||0),Number(x.income||0),Number(x.cost_percentage||0),Number(x.personnel_cost||0),Number(x.basic_expenses||0),Number(x.net||0),x.notes||'',(x.items||[]).map(i=>`${i.item_name}: ${i.quantity||0} ${i.unit||''} · $${i.total_cost||0}`).join(' | ')])];
    cmSendXlsx(res,`CM_costos_${month}.xlsx`,`Costos ${month}`,rows);
  }catch(e){console.error(e);res.status(500).json({error:'No se pudo generar el Excel de costos.'})}
});
