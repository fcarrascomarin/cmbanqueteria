...
async function refreshConsultationProgress(db=pool){
  ...
  SET status=CASE
      ...
      WHEN e.has_record OR e.has_document OR e.has_direct_document THEN 'completado'
      ELSE 'pendiente'
    END,
    document_date=CASE
    ...
    updated_at=NOW()
  FROM evidence e
  WHERE d.id=e.id AND d.status<>'bloqueado'`)

  await db.query(`WITH stats AS (
    SELECT m.id,COUNT(d.id)::int total,COUNT(d.id) FILTER(WHERE d.status='completado')::int done
  ...
  SET status=CASE
    ...
    WHEN s.done>0 AND m.status NOT IN ('realizado','completado','bloqueado','pendiente_posterior') THEN 'en_curso'
    ELSE m.status END,
